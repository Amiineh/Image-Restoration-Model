...
class ImageRestorationDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir) -> None:
        super().__init__()
        self.original_path = os.path.join(root_dir, "src_imgs")
        self.corrupted_path = os.path.join(root_dir, "corrupted_imgs")
        self.mask_path = os.path.join(root_dir, "binary_masks")

        self.original_img_files = sorted(os.listdir(self.original_path))
        self.corrupted_img_files = sorted(os.listdir(self.corrupted_path))
    ...
    def __getitem__(self, index):
        original_img_path = os.path.join(self.original_path, self.original_img_files[index])
        corrupted_img_path = os.path.join(self.corrupted_path, self.corrupted_img_files[index])
        mask_path = os.path.join(self.mask_path, self.mask_files[index])

        # load images using existing functions:
        original_img = read_rgba_img(original_img_path)
        corrupted_img = read_rgba_img(corrupted_img_path)

        # normalize images:
        original_img = normalize_rgb_img(original_img)
        corrupted_img = normalize_rgb_img(corrupted_img)

        # Load and process mask
        mask = np.load(mask_path).astype(np.float32)
        mask = torch.from_numpy(mask)
        if mask.ndim == 2:
            mask = mask.unsqueeze(0)  # Add channel dimension if needed

        return torch.from_numpy(original_img), torch.from_numpy(corrupted_img), mask


# # testing Dataset class:
# dataset = ImageRestorationDataset("../validation/")
# corrupted_img, original_img, mask = dataset[0]
# print(f"Shapes - Corrupted: {corrupted_img.shape}, Original: {original_img.shape}, Mask: {mask.shape}")
